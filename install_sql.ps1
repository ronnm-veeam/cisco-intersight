...
if ($sqltest.count -eq 0 ) {
    ...
    Write-Host "Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile | Wait-Process" *>> $logdir"\install_sql.log"
    Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile | Wait-Process *>> $logdir"\install_sql.log"
