...
if ($sqltest.count -eq 0 ) {
    ...
    Write-Host "Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -Wait" *>> $logdir"\install_sql.log"
    Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -Wait *>> $logdir"\install_sql.log"
