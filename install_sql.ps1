...
if ($sqltest.count -eq 0 ) {
    ...
    Write-Host "Start-Process -FilePath $sqlsetup -arg $arguments -LoadUserProfile -NoNewWindow-WorkingDirectory $logdir -Wait" *>> $logdir"\install_sql.log"
    Start-Process -FilePath $sqlsetup -arg $arguments -LoadUserProfile -NoNewWindow -WorkingDirectory $logdir -Wait *>> $logdir"\install_sql.log"
