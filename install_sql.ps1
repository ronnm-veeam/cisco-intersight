...
if ($sqltest.count -eq 0 ) {
    ...
    Write-Host "Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile -WorkingDirectory $logdir -Wait" *>> $logdir"\install_sql.log"
    Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile -WorkingDirectory $logdir -Wait *>> $logdir"\install_sql.log"
