...
if ($sqltest.count -eq 0 ) {
    ...
    Write-Host "Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile -NoNewWindow-WorkingDirectory $logdir -Wait" *>> $logdir"\install_sql.log"
    Start-Process -FilePath $sqlsetup -arg $arguments -PassThru -LoadUserProfile -NoNewWindow -WorkingDirectory $logdir -Wait *>> $logdir"\install_sql.log"
